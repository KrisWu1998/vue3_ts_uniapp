<template>
  <view class="myMessage-box">
    ...
    <view class="back" @tap="handleGoBack()">
      ...
      返回
      <view class="back-title"> 消息 </view>
    </view>
    <!-- 选项卡 -->
    <Tabs :tabs="tabs" v-model:activeIndex="activeIndex" class="tabs-box" />
    <!-- 已读操作栏 -->
    <view class="handle-box">
      ...
      >
        <image :src="item.url" class="handle-item-icon" />
        {{ item.label }}
      </view>
    </view>
    <!-- 消息列表 -->
...
export default defineComponent({
  ...
  setup() {
    ...
    const state = reactive({
      messageWrapDom: HTMLDivElement, // 内容区Dom
      domScrollTop: 0,
      loading: false,
      ...
      globalData: {
        1: [
          ...
          },
          {
            ...
            title: "节日赠租币",
            content:
              "人人租机祝你节日快乐，送88个租币位置送88个租币位置送88个租币位置",
            time: "2020-03-23 14:30",
            isRead: false,
          },
        ],
    ...
    const methods = reactive({
      ...
      handleRead(index: number): void {
        const message =
          index === 1 ? "是否确认要已读全部通知" : "是否确认要删除已读通知";
      ...
      runningDeleteOrRead(index: number): void {
        ...
        index === 1
          ? this.handleAllRead(activeIndex, id)
          : this.deleteCanRead(activeIndex, id);
      },
      // 删除全部已读
      deleteCanRead(activeIndex: number, id: number): void {
        ...
        setTimeout(() => {
          ...
          const newList =
            globalData[id].filter((item: any) => !item.isRead) || [];
      ...
      handleScroll() {
        const dom = state.messageWrapDom;
        state.domScrollTop = (dom as any)?.scrollTop as any;
      },
    });

    onMounted(() => {
      (state as any).messageWrapDom = document.querySelector(
